...
def game_over():  # function to handle game over state
    ...
    if exit_button.update():
        pygame.quit()
        exit()
    if replay_button.update():
        player.lives = 3
        player.alive = True
        player.spawned = False
        player.rect.center = (440, 500)
...
class Player(
    ...
    def __init__(self, images, x, y):
        ...
        self.playerChannel.set_volume(0.5)
        self.flapsound = pygame.mixer.Sound("joust_flap.wav")
        self.spawnsound = pygame.mixer.Sound("joust_spawn.wav")
        self.lavasound = pygame.mixer.Sound("joust_lava.wav")
        self.collidesound = pygame.mixer.Sound("joust_collide.wav")
    ...
    def lava(self):
        ...
        if self.lavatime == 0:
            self.playerChannel.play(self.lavasound)
        if not self.lavaDeath:
            return

        if self.lavatime < ANIMATION_COOLDOWN:
            self.rect.y += 1
            self.lavatime += 1
        else:
            self.lavaDeath = False
    ...
    def update_animation(self):
        if self.lavaDeath:
            self.lava()
        if not self.alive and self.lives > 0 and not self.lavaDeath:
            self.spawn()
    ...
    for event in pygame.event.get():  # Handle events
        ...
        if event.type == pygame.KEYDOWN:  # Handle key presses
            ...
            if (
                ...
                player.flying = True
                player.flap_count += 1
                player.playerChannel.play(player.flapsound)
                player.vel_y = -10
